...
import StudentProfileEditor from "../../components/students/StudentProfile";

const StudentDashboard = () => {
  const { currentUser, logout } = useAuth();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState("profile");
  const [isEditingProfile, setIsEditingProfile] = useState(false); // State to manage edit profile visibility
  ...
  const handleEditProfileClick = () => {
    setIsEditingProfile(true); // Show the StudentProfileEditor component
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center bg-white rounded-xl shadow p-4 mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            className={`px-4 py-2 rounded-xl ${activeTab === "chats" ? "bg-[#C3A1FF] text-white" : "bg-white"}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <AnimatedSection>
            <div className="p-6 bg-white shadow rounded-xl">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="mt-2 text-sm text-gray-500">{currentUser?.institutionName}</p>
                <p className="text-sm text-gray-500">{currentUser?.graduationYear ? `Class of ${currentUser.graduationYear}` : "Current Student"}</p>
                <p className="text-sm text-gray-500">{currentUser?.bio}</p>

                <button 
                  className="w-full mt-4 px-4 py-2 border rounded-xl hover:bg-gray-100"
              ...
              <div className="mt-6 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{currentUser?.major || "Computer Science"}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{currentUser?.institutionName || "State University"}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{currentUser?.location || "San Francisco, CA"}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="md:col-span-2">
          {/* Show StudentProfileEditor if isEditingProfile is true */}
          {isEditingProfile ? (
            <AnimatedSection delay={100}>
              <StudentProfileEditor 
                onClose={() => setIsEditingProfile(false)} // Close the editor
              />
            </AnimatedSection>
          ) : (
            <>
            ...
            <AnimatedSection delay={100}>
              <div className="bg-white shadow rounded-xl">
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500">{currentUser?.institutionName || "State University"}</p>
                        <p className="text-sm text-gray-500">
                          {currentUser?.graduationYear ? `Class of ${currentUser.graduationYear}` : "Current Student"}
                        </p>
                ...
                <div className="p-6 border-t">
                  ...
                  <p className="text-gray-500">{currentUser?.bio || "No bio available."}</p>
                </div>
              </div>
            </AnimatedSection>
          )}
